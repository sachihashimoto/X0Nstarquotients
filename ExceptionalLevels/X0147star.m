...
function classify_points_on_X0N_wd(N, wds)
    ...
    for pt in pts do
        ...
        try
            ...
            if tf then
                printf "P = %o (j = %o): CM by %o\n", P, jP, D;
            else
                printf "P = %o (j = %o): not CM\n", P, jP;
            end if;
        catch e 
            printf "ERROR: %o notin Q, minpoly = %o.\n", jP, MinimalPolynomial(jP);
            printf "j(P)'s: %o\n", [<j(P), MinimalPolynomial(j(P)[1])> : P in PointsOverSplittingField(pt@@quotient)];
        end try;*/
    end for;
...
SetLogFile("X0147star.log");
printf "X_0(147)^*:\n";
X := classify_points_on_X0N_wd(N, [1,3,49,147]);
