...
function clunk(N,A : A2 := [], Raw:=false, Reduce:=false)
 ...
 if #A ne 0 then vprintf ModularCurve: "Quotienting by Atkin-Lehner\n";
  AL:=[AtkinLehnerOperator(M,al) : al in A cat A2]; J,T:=Diagonalization(AL);
  //print J;
  B:=ChangeRing(Matrix([B]),PowerSeriesRing(Rationals()));
  B:=Eltseq(ChangeRing(T,Parent(B[1][1]))*Transpose(B));
  indices := [i : i in [1..#B] | &and[a[i][i] eq +1 : a in J[1..#A]]];
  //indices2 := [i : i in [1..#B] | &and[a[i][i] eq +1 : a in J]];
  B:=[B[i] : i in indices]; end if; g:=#B;
 ...
 C:=Curve(P,EQ); assert Degree(C) eq 2*g-2;
 auts := [];
 //print C;
 /*
 for wd in J[#A+1..#A+#A2] do
  aut := [wd[j,j]*x[i] : i -> j in indices];
  print aut;
  Append(~auts, iso<C -> C | aut, aut>);
 end for;*/
 for s in Subsets({1..#indices}) do // all possible involutions
  if #s in {0,#indices} then
    continue;
  end if;
  try
    aut := [(i in s select +1 else -1)*x[i] : i in [1..#x]];
    Append(~auts, iso<C -> C | aut, aut>);
  catch e;
  end try;
 end for;
 //print auts;
 //assert #auts eq 2^#A2;
 autX := AutomorphismGroup(C, auts);
 D, pi := CurveQuotient(autX);
 assert Genus(D) eq 1;
 assert Degree(pi) eq #autX; // make sure we haven't quotiented out by an extra isogeny
 ratpts := PointSearch(D, 10);
 E := EllipticCurve(D, ratpts[1]);
 E := SimplifiedModel(E);
 return E;
end function;
...
for Ng in /* [
  ...
  for i in [1..#seq_al] do
    try
      E := clunk(N, seq_al[[j : j in [1..#seq_al] | j ne i]] : A2 := seq_al[[i..i]]);
      if (Genus(E) ne 1) or (not IsDivisibleBy(N, Conductor(E))) then 
        continue;
      end if;
      printf "N = %o: #E(Q) = %o\n", N, #MordellWeilGroup(E);
      print E;
      /*wds := [[d : d in Divisors(N) | Gcd(d, ExactQuotient(N,d)) eq 1]];
      X, ws, pairs, NB, cusp := eqs_quos(N, wds);
      if not IsIsomorphic(E, X) then
        print E, X;
      end if;*/
      Exclude(~Ns, N);
      break;
...
for N in Ns cat [96,72] do 
  wds := [[d : d in Divisors(N) | Gcd(d, ExactQuotient(N,d)) eq 1]];
  X, ws, pairs, NB, cusp := eqs_quos(N, wds);
  E := pairs[1,1];
  E := EllipticCurve(E, PointSearch(E, 10)[1]);
  printf "N = %o: #E(Q) = %o\n", N, #MordellWeilGroup(E);
  print E;
end for;