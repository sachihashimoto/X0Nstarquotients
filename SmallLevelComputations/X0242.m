...
// function classify_points_on_X0N_wd(N, wds)
    ...
    for pt in pts do
        newpts := PointsOverSplittingField(pt@@quotient); // only [1]: all points correspond to isogenous curves
        printf "pt = %o: \n j's = %o\n", pt, {printj(j(ptj)) : ptj in newpts};
    end for;
...
// X := classify_points_on_X0N_wd(N, [ 1, 2, 121, 242]);
